...
void user::display_msgs(string title, msg *head)
{
	...
	else
	{
		...
		cout
				<< "\n--------------------------------------------------------------------------";
		cout << "\n" << setw(5) << "Sr no." << setw(15) << "From" << setw(15)
				<< "To" << setw(15) << "Message" << setw(10) << "Read"
				<< setw(10) << "Starred";
		cout
				<< "\n--------------------------------------------------------------------------";
		...
		while (ptr != NULL)
		{
			cout << "\n" << setw(5) << i << setw(15) << ptr->from << setw(15)
					<< ptr->to << setw(15) << ptr->text.substr(0, 8) << "..."
					<< setw(10) << ptr->read << setw(10) << ptr->star;
			cout
					<< "\n--------------------------------------------------------------------------";
			ptr = ptr->link;
			i++;
...
class messager
{
	...
	public:
		...
		//void remove(user *ptr);
		void remove(string userName , string userWord);		//to delete your account
...
void messager:: remove(string userName , string passWord)
{

    user *pre_node;

   if(start==NULL) {
      cout<<"No accounts are present ";
      return;
   }

   if(start->username== userName && start->password == passWord) {
      if(start->next != NULL) { // not a last node
   ...
   } else if(start->username != userName  && start->password != passWord && start->next == NULL) {
   ...
   while(current->next != NULL && current->username != userName && current->password != passWord) { // traverse the list
      pre_node = current;
   ...
   if(current->username== userName && current->password  == passWord) {
      ...
      cout<<" not found in the list."<<userName<<" "<<passWord;
}
